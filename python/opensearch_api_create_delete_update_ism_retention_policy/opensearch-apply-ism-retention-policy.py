...
SITE_INFO_FILE_PATH = '/opt/tps/deployment/site-info.yml'
active_ism_policies = []
...
def delete_policy(policy_name):
    ...
    if delete_response.status_code == 200:
        ...
        print('policy {policy_name} was not created - please check connectivity to the server ,check if index prefix already exsists or other policy parameters')
...
def update_policy_template(policy_name,min_index_age_in_days):
    policy_template = '''{
        "policy": {
            "description": "dudu index retention flow",
            "default_state": "hot",
            "schema_version": 1,
            "states": [
                {
...
def create_policy(policy_name,min_index_age_in_days):
    ...
    if create_response.status_code == 201:
        ...
        logging.error(f'policy {policy_name}  was not created - please check connectivity to the server ,check if index prefix already exsists or other policy parameters')
        logging.error(f'status code is : {create_response.status_code} and status content is {create_response.content}')
        return False
...
def update_policy(policy_name,min_index_age_in_days,policy_sequential_number,policy_primary_term):
    ...
    if update_response.status_code == 200:
        ...
        logging.error(f'Policy {policy_name} was not updated - Please check if opensearch_main_ism_retention_policy section under site-info file includes multiple index name ')
        logging.error(f'status code is : {update_response.status_code} and status content is {update_response.content}')
        return False
    else:
...
get_policy_details = get_policies()
...
for policy in list_of_ism_retention_policies_to_upload:
    if (policy['status']) == 'disable':
        ...
        logging.error('something is wrong - Please check the "opensearch_main_ism_retention_policy" section under site-info.yml file')
