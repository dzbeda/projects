...
resource "aws_instance" "external_server" {
  count = 1
  ami               = var.ami_id
  instance_type     = var.instance_type
  subnet_id = aws_subnet.public_subnet.*.id[count.index]
  vpc_security_group_ids  = [aws_security_group.external_server.id]
...
## Some server security group
resource "aws_security_group" "external_server" {
  name ="external-server-security-group"
  vpc_id = aws_vpc.vpc-main.id
  ## Incoming roles
  ingress {
    from_port = 22
